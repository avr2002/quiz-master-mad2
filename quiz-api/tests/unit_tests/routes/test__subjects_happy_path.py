...
@pytest.mark.skip(
    reason="Known SQLite in-memory database corruption issue - (sqlite3.DatabaseError) database disk image is malformed"
)
def test_update_subject_as_admin(client: FlaskClient, admin_token: str, subject: Subject) -> None:
